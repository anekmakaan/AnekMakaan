...
const vtIframeEnquiry = (e, src, sec = 5) => {
  if(src){
    $(e).empty().append('<iframe frameborder="0" class="d-video" allowfullscreen="" sandbox="allow-forms allow-scripts allow-pointer-lock allow-same-origin allow-top-navigation" allow="autoplay" src="' + src + '?autoplay=1&end=' + sec + '"></iframe>');
    $timeOut = (sec * 1000) + 2000;
    setTimeout(() => $('#enqModal').modal('show'), $timeOut);
  }
}
